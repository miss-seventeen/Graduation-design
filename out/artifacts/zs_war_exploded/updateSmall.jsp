...
<%--<body>
<form action="updateSmall" method="post">
    <input type="hidden" name="sid" value="${smallById.sid}">
    小类类名：<input type="text" name="sname" value="${smallById.sname}"/> <br/>
    &lt;%&ndash; 小类名称： <c:forEach items="${byIdSmall}" var="small">

    </c:forEach>   &ndash;%&gt;
    大类名称：<select name="bsid.bid">
   <c:forEach items="${allBig}" var="big">
        <option value="${big.bid}">${big.bname}</option>
    </c:forEach>
    &lt;%&ndash;  <option value="${small.sid}">${small.sname}</option> &ndash;%&gt;
</select>
        ...
        <div class="quick-actions_homepage">
            <ul class="quick-actions">
                <form action="updateSmall" method="post">
                    ...
                    <input type="hidden" name="sid" value="${smallById.sid}">
                    小类类名：<input type="text" name="sname" value="${smallById.sname}"/> <br/>
                    <%-- 小类名称： <c:forEach items="${byIdSmall}" var="small">

                    </c:forEach>   --%>
                    大类名称：<select name="bsid.bid">
                    <c:forEach items="${allBig}" var="big">
                        <option value="${big.bid}">${big.bname}${big.btype==1?"支出":"收入"}</option>
                    </c:forEach>
                    <%--  <option value="${small.sid}">${small.sname}</option> --%>
                </select>
                    <input type="submit" value="修改">
                </form>
            </ul>
