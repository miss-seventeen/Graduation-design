...
<table border="1">
    ...
    <c:forEach items="${alluser}" var="users">
        <tr>
            <td>${users.uid }</td>
            <td>${users.uname }</td>
            <td>${users.uflag==1?"管理员":"用户" }</td>
            <td>
                <c:if test="${users.uflag == 0 }">
                    <a href="delUser?uid=${users.uid }">删除</a>
                </c:if></td>

            <td> <a href="byIdUser?uid=${users.uid }">修改</a></td>
        </tr>
    </c:forEach>
</table>
</body>
    ...
    <div class="container-fluid">
         <div class="quick-actions_homepage" style="width: 80%;overflow-x: scroll;margin: 40px 0 0 30px;">
             <ul>
               ...
               <%-- <li class="bg_lo span3"> <a href="allUser"> <i class="icon-th-list"></i>用户表</a> </li>
                ...
                <table id="table" cellspacing="0" cellpadding="2" width="100%" border="1">
                    ...
                    <c:forEach items="${alluser}" var="users">
                        <tr>
                            <td>${users.uid }</td>
                            <td>${users.uname }</td>
                            <td>${users.uflag==1? "家主":"成员"}</td>
                            <td>
                                <c:if test="${user.uflag==1}">
                                    <button><a href="delUser?uid=${users.uid }&&uflag=${users.uflag}"class="button button2" name="t">删除</a></button>
                                </c:if></td>

                            <td> <c:if test="${user.uid==users.uid}"><button><a href="byIdUser?uid=${users.uid }">修改</a></button></c:if></td>
                        </tr>
                    </c:forEach>
                </table>
                 <a href="exportUser">导出用户</a><br/>
