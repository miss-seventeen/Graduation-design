                ...
                <li>
                    ...
                    <table id=table cellspacing=0 cellpadding=2 width=100% border=1>
                        ...
                        <c:forEach items="${listRecord}" var="record">
                            <tr>
                                <td>${record.rid}</td>
                                <td>${record.rsid.sname}</td>
                                <td>
                                        ${record.rsid.bsid.btype==1?"收入":"支出"}
                                </td>
                                <td>${record.rtime}</td>
                                <td>${record.money}</td>
                                <td>${record.runame}</td>
                                <td>${record.uid.uname}</td>
                                <td>${record.wtime}</td>
                                <td>${record.beizhu}</td>
                                <td>
                                    <button> <a href="delRecord?id=${record.rid}" onclick="return aaa()">删除</a></button></td>
                                <td> <button> <a href="byIdRecord?id=${record.rid}">修改</a></button></td>
                            </tr>
                        </c:forEach>

                        <tr>
                            <button> <a href="exportRecord">导出记录</a></button>
                        </tr>
                        <tr><form action="zzt" method="post">
                            输入查询日期：
                            <input class="custome_birthday" type="date" placeholder="起始日期" name="time">~
                            <input class="custome_birthday" type="date" placeholder="结束日期" name="tim">&nbsp;&nbsp;
                            <input type="submit" style="height: 30px" value="查看柱状图">
                        </form>
                        </tr>

                    </table>
                    <tr>
                        <td><button><a href="recordListPage">首页</a></button></td>
                        <td><button><a href="recordListPage?page=${page-1}" onclick="return aa(${page-1})">上一页</a></button></td>
                        <td><button><a href="recordListPage?page=${page+1}" onclick="return cc(${page})">下一页</a></button></td>
                        <td><button><a href="recordListPage?page=${lastPage}">尾页</a></button></td>
                    </tr>
                    ...
                    <form action="recordListPage" method="post" onsubmit="return bb()">
